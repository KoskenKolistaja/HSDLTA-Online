...
func _process(delta: float) -> void:
	if camera_rot_smoothing:
		...
		look_input_smoother_tween.tween_property(
			self, "smoothed_look_input", look_input, camera_rot_smoothing
		)
...
func _physics_process(delta):
	...
	if Input.is_action_pressed("mouse2"):
		$HeadPivot/Camera3D/Viewmodel.position = $HeadPivot/Camera3D/Viewmodel.position.move_toward(
			WEAPON_AIM_POS, 0.01
		)
	else:
		$HeadPivot/Camera3D/Viewmodel.position = $HeadPivot/Camera3D/Viewmodel.position.move_toward(
			WEAPON_DEFAULT_POS, 0.01
		)
